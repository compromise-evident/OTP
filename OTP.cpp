...
/// Version 3, plausibility ratio = 1:95, number of plausible messages per ciphertext = 165,153,743,850,136,394,768,170,261,844,001,846,794,583,739,849,096,139,995,229,484,616,386,098,260,836,764,841,563,284,148,593,084,838,704,354,991,162,901,768,186,888,062,928,119,325,079,023,838,043,212,890,625
...
/*Generating keys requires 1+GB RAM.  1,600,000 outgoing messages per user.  Generate and share multiple folders to have
spare keys. Keys are truncated upon use. Plaintext is 80 characters max, randomness is appended to all plaintext to get 80
characters. Ciphertext is always 80 characters.  The following 5,123 files conjure speedy automated dual-channel symmetry:
-------------------------------------------------------------------------------------------------------------------------
 * keys                    (folder)
...
{	cout << "\n(One-time pad)\n\n"
	...
	{	ifstream in_stream;
		...
		{	     if(plaintext[a] == ' ') {ciphertext[a] =  0;}	else if(plaintext[a] == 'P') {ciphertext[a] = 48;}
			...
			else if(plaintext[a] == 'O') {ciphertext[a] = 47;}	else if(plaintext[a] =='\0') {ciphertext[a] = -1;} //Append rand marker.
		...
		//Checks if file symmetry.entanglement exists.
		...
		in_stream.close();
		if(sniffed_one_file_character_symmetry_entanglement[0] == true)
		{	sniffed_one_file_character_symmetry_entanglement[0] = true;
		}
		...
		{	int temp_file_numbering = file_numbering;
			file_name_key_outgoing[19] = (temp_file_numbering % 10); //       ./keys/outgoing/0  0  0  0    (file name)
			temp_file_numbering /=    10; //                                                  ^  ^  ^  ^
			file_name_key_outgoing[18] = (temp_file_numbering % 10); //                      16 17 18 19    (element layout)
			temp_file_numbering /=    10;
			file_name_key_outgoing[17] = (temp_file_numbering % 10);
			temp_file_numbering /=    10;
			file_name_key_outgoing[16] = (temp_file_numbering % 10);
			
			for(int b = 16; b <= 19; b++) {file_name_key_outgoing[b] += 48;} //Fixes file name.
			
			file_numbering++;
			
			in_stream.open(file_name_key_outgoing);
			char sniffed_one_file_character[1];
			in_stream.get(sniffed_one_file_character[0]);
			in_stream.close();
			
			if(sniffed_one_file_character[0] != '\0') {break;}
		}
		...
		int  truncating_table_private_null_bookmark;
	...
	{	ifstream in_stream;
		...
		for(int a = 0; a < 10000; a++)
		{	int temp_file_numbering = file_numbering;
			file_name_key_incoming[19] = (temp_file_numbering % 10); //       ./keys/incoming/0  0  0  0    (file name)
			temp_file_numbering /=    10; //                                                  ^  ^  ^  ^
			file_name_key_incoming[18] = (temp_file_numbering % 10); //                      16 17 18 19    (element layout)
			temp_file_numbering /=    10;
			file_name_key_incoming[17] = (temp_file_numbering % 10);
			temp_file_numbering /=    10;
			file_name_key_incoming[16] = (temp_file_numbering % 10);
			
			for(int b = 16; b <= 19; b++) {file_name_key_incoming[b] += 48;} //Fixes file name.
